            ...
            foreach ( $wc_items as $wc_cart_item ) {
                ...
                if ( $wc_cart_item->getStaff() && $candidate_cart_item->getService() ) {
                    if ( $candidate_staff_id == $wc_cart_item->getStaff()->get( 'id' ) ) {
                        // Equal Staff
                        $candidate_begtime = $candidate_slots[0][2];
                        $candidate_endtime = $candidate_begtime + $candidate_cart_item->getService()->get( 'duration' ) + $candidate_cart_item->getExtrasDuration();
                        $wc_cart_begtime   = $wc_cart_slots[0][2];
                        $wc_cart_endtime   = $wc_cart_begtime + $wc_cart_item->getService()->get( 'duration' ) + $wc_cart_item->getExtrasDuration();
                        if ( ( $wc_cart_endtime > $candidate_begtime ) && ( $candidate_endtime > $wc_cart_begtime ) ) {
                            // Services intersected.
                            if ( $candidate_begtime == $wc_cart_begtime ) {
