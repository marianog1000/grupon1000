...
class Controller extends Lib\Base\Controller
{
    ...
    public function renderShortCode( $attributes )
    {
        ...
        Lib\Stats::recordForm( $_SERVER[ 'REQUEST_URI' ] );
        ...
        if ( $loaded ) {
            ...
            foreach ( $at->getSlots() as $group => $group_slots ) {
                $slots[ $group ] = preg_replace( '/>\s+</', '><', $this->render( '_time_slots', array(
                    'group' => $group,
                    'slots' => $group_slots,
                    'duration_in_days' => $at->isServiceDurationInDays(),
    ...
    public function executeRenderRepeat()
    {
        ...
        if ( $userData->load() ) {
            ...
            if ( $repeat_data ) {
                ...
                $client_diff = \Bookly\Lib\Config::useClientTimeZone()
                ...
                $schedule = Lib\Proxy\RecurringAppointments::buildSchedule(
                    clone $userData,
                    date( 'Y-m-d H:i:s', $slots[0][2] - $client_diff ),
                    ...
                    if ( $attr['show'] ) {
                        switch ( $column ) {
                            ...
                            case 'price':
                                ...
                                break;
                            case 'deposit':
                                if ( Lib\Config::isDepositPaymentsEnabled() ) {
                                    $cart_items[ $cart_key ][] = apply_filters( 'bookly_deposit_payments_format_deposit', $cart_item->getDepositPrice(), $cart_item->getDeposit() );
                                    $deposit['show'] = true;
                                }
                                break;
                        }
